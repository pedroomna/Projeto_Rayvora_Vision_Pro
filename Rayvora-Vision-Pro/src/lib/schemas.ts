...
export const CattleRecordSchema = z.object({
  ...
  animalId: z.string().min(1, "O número do brinco é obrigatório"),
  ...
  score: z.preprocess(
    ...
    z.number()
    .min(1.0, "O valor mínimo permitido é 1.0")
    .max(5.0, "O valor máximo permitido é 5.0")
  ),
  weight: z.preprocess(
    (val) => (val === '' || val === null || val === undefined ? undefined : Number(val)),
    z.number()
...
export function validateCattleRecord(data: unknown) {
  try {
    ...
    if (error instanceof z.ZodError) {
      const messages = error.issues.map(err => `${err.path.join('.') || 'campo'}: ${err.message}`).join('\n');
